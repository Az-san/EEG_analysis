...
for electrode in electrodes:
    ...
    if os.path.exists(file_path):
        ...
        plt.axvline(0, color="brown", linestyle="--", label="TTL Signal")
        plt.axhline(0, color="black", linestyle="-", linewidth=0.8)
        plt.title(f"{electrode} Error Trial")
        plt.xlabel("Time [ms]")
        plt.ylabel("Amplitude [μV]")
        plt.xlim(-1000, 2000)
        plt.ylim(-7, 7)
        plt.grid(True)
        plt.legend()
        error_plot_path = os.path.join(plot_output_dir, f"{electrode}_error.png")
        plt.savefig(error_plot_path, dpi=300)
        plt.close()
        print(f"{error_plot_path} に保存しました。")

        # ✅ 比較プロット（Correct, Error, Difference）
        plt.figure(figsize=(10, 6))
        plt.plot(data["Time [ms]"], data["Correct Average [μV]"], label="Correct", color="blue")
        plt.plot(data["Time [ms]"], data["Error Average [μV]"], label="Error", color="red")
        ...
        plt.axvline(0, color="brown", linestyle="--", label="TTL Signal")
        plt.axhline(0, color="black", linestyle="-", linewidth=0.8)
        plt.title(f"{electrode} Comparison")
        plt.xlabel("Time [ms]")
        plt.ylabel("Amplitude [μV]")
        plt.xlim(-1000, 2000)
        plt.ylim(-7, 7)
        plt.grid(True)
        plt.legend()
        comp_plot_path = os.path.join(plot_output_dir, f"{electrode}_comp.png")
